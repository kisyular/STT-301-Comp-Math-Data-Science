...
#z = WorldBank$region2[WorldBank$region2 == "LatAmer"]
...
table(WorldBank$region2, WorldBank$life.expectancy > 60, dnn = c("Life Expectancy more than 60 years", "Life Expectancy of Different Regions of the World"))
```
...
with(WorldBank, table(region2, life.expectancy > 60, dnn = c("Life Expectancy more than 60 years", "Life Expectancy of Different Regions of the World")))
```
...
+ $23.5 \leq lat \leq 66.5$: Northern temperate
+ $0 \leq lat < 23.5$: Northern tropics
+ $-23.5 \leq lat < 0$: Southern tropics
+ $-66.5 \leq lat < -23.5$: Southern temperate
...
index = with(WorldBank, cut(latitude, breaks = c(-66.5, -23.5, 0, 23.5, 66.5), labels = c("Southern temperate", "Southern tropical", "Northern tropical", "Northern temperate"), include.lowest = TRUE,  right = FALSE))

tapply(WorldBank$life.expectancy, INDEX = index, FUN = mean, na.rm=TRUE)
...
*Response: The class is really educative. I believe I am learning a lot.*
