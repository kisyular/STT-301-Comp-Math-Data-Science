...
```{r}
# TODO: Extract the third row here
gapminder[3,]
```
...
```{r}
# TODO: Extract the first 50 elements of the variable year here
...
```{r}
# TODO: Display the unique values in the variable year here
...
```{r}
# TODO: Create the data frame here
...
``` {r}
# Yes, there is obvious differences between the two years. For example, In 1952 the life expectancy of Americas was pretty less compared to 2002
```

Next calculate the median life expectancy, for both 1952 and 2002. 

```{r}
# TODO: Calculate the median life expectancies here
median(gapminder1952[,'lifeExp'])
median(gapminder2002[, 'lifeExp'])

#Answer: The median life expectancy in year 1952 was 45.1355 while in 2002 was 70.8255
```

What do you notice?
```{r}
# There is a significant difference between the two years. It seems life expectancy went up
```

Next calculate the mean and median GDP for both years. 
```{r}
# TODO: Calculate the mean and median GDPs here
median(gapminder1952[,'gdpPercap'])
median(gapminder2002[, 'gdpPercap'])
#ANswer: The Median for GDP for the year 1952 was 1968.528 and for the yaer 2002 was 5319.805

mean(gapminder1952[,'gdpPercap'])
mean(gapminder2002[, 'gdpPercap'])
# The Mean for GDP for the year 1952 was 3725.276 and for the yaer 2002 was 9917.848
```

Again, what do you notice?
``` {r}
# The Mean and Media for the GDP in  1952 almost thriced in year 2002
```
...
```{r}
# TODO: Create the data frame and draw the scatter plots here
gapminderKenya = subset(gapminder, country == 'Kenya')

year = gapminderKenya[, 'year']
lifeExp = gapminderKenya[, 'lifeExp']
scatter.smooth(lifeExp, year, ylab = "Time in Years", xlab = "Life expectacy in Kenya")
```

```{r}
GDPKenya = gapminderKenya[, 'gdpPercap']
scatter.smooth(GDPKenya, year, ylab = "Time in Years", xlab = "GDP of Kenya")
```
...
```{r}
# TODO: Enter 0.2 == 0.6/3 and see what happens
0.2 == 0.6/3
...
point3==0.3
# Anser: Not all values where equal to 0.3
```
