...
```{r}
casino.na <- read.csv(file = "nevada_casino_sqft.csv", header = TRUE, na.strings = " ", stringsAsFactors = FALSE)
...
```{r}
apply(casino.na, MARGIN = 1, function(x) sum(is.na(x)))
```

2. find the mean square footage for “PITGAMES”, “SLOTS”, “KENO”, “BINGO”, “SPORTS”, “POKER”, and
“TOTAL”;

```{r}
apply(X = casino.na[, c("PITGAMES", "SLOTS", "KENO", "BINGO", "SPORTS", "POKER", "TOTAL")], MARGIN = 2, function(x) mean(x, na.rm=TRUE))
```

3. find the median square footage for "PITGAMES", "SLOTS", "KENO", "BINGO",
```{r}
apply(X = casino.na[, c("PITGAMES", "SLOTS", "KENO", "BINGO", "SPORTS", "POKER", "TOTAL")], MARGIN = 2, function(x) median(x, na.rm=TRUE))
```

4. compute the five number summary (use the function `summary`) for "PITGAMES", "SLOTS", "KENO", "BINGO", "SPORTS", "POKER", and "TOTAL";
```{r}
apply(X = casino.na[, c("PITGAMES", "SLOTS", "KENO", "BINGO", "SPORTS", "POKER", "TOTAL")], MARGIN = 2, function(x) summary(x, na.rm=TRUE))
```
...
```{r}
square_root <- function(col) {
    return (IQR(col, na.rm = T))
}

apply(X = casino.na[, c("PITGAMES", "SLOTS", "KENO", "BINGO", "SPORTS", "POKER", "TOTAL")], MARGIN = 2, function(x) square_root(x))
```
