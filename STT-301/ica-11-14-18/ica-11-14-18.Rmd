...
match("skilling",enron)
enron %>% { which(. == "skilling") }
...
```{r}
# which element is "CHAPTER 1"?
match("CHAPTER 1",SandS)

# which element is "THE END"?
match("THE END", SandS)

# remove all text before "CHAPTER 1" and after "THE END". Verify this was done correctly using the head and tail functions.
chapter.1.the.end <- SandS[match("CHAPTER 1", SandS): match("THE END", SandS)]

# use the paste function to create one long string of the novel as opposed to multiple lines with separate strings. Verify that the length is now 1. Display the struture using str.
