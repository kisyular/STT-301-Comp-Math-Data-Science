...
```{r}
system.time(rand_walk_2(1000000))
```
...
rand_walk_4 <- function(n) {
    ...
    for(i in 2:n){
      ...
      if (move_horiz[i-1]) {
        ##Make the move, either +1 or -1
        xpos[i] <- xpos[i-1] 
        ypos[i] <- ypos[i-1] + move_directions[i - 1]
      } ##If not horizontal, then vertical
      else {
        xpos[i] <- xpos[i-1]
        ypos[i] <- ypos[i-1] + move_directions[i - 1]
...
system.time(rand_walk_4(1000000))
```
...
```{r}
system.time(rand_walk_5(1000000))
```
...
```{r}
system.time(rand_walk_6(1000000))
```
