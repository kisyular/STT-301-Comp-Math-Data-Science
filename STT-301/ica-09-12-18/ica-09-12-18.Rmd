...
** R is will drop the last 4 elements and return the other elements in reverse order**

```{r}
vector_test = c(44,50,68,77,88,99,20,33,45,90)
vector_test[length(vector_test) - 2]

vector_test[ 10 -2 : length(vector_test)]
```
...
Compute the average winning time over the most recent 10 years for each of the three races.
```{r}
# Compute the means here..

mean(belmont[(length(belmont) - 10):length(belmont)])
```
...
# Mimic the above for the two other races and save the variables as tc.winner.preakness and tc.winner.belmont
```
...
```{r}
# Create bar plot here for tc.winner.preakness
```

```{r}
# Create bar plot here for tc.winner.belmont
```
