---
title: "STT 301 Homework Assignment 3 Solutions"
author: "Shawn Santo"
...
The `tb_cases.csv` file (available on D2L - Data Sets section) contains tuberculosis (TB) cases by country, year, age, gender, and diagnosis method. The data is from 1980 to 2013. A data dictionary is available at http://www.who.int/tb/country/data/download/en/.

The objective in question 1 is to make the data tidy. Each of the subsequent parts will help you in the process of tidying the data. The resulting tibble is shown (by default only 10 rows) for most of the parts. You must use the functions in the `tidyr` package and `dplyr` package (both of which are loaded when you load the `tidyverse` package) along with the pipe operator (where applicable) to earn full credit. You will also need to load the
`stringr` package.
...
```{r echo = T}
tb.cases <- read.csv(file="tb_cases.csv", header = TRUE, stringsAsFactors = FALSE, na.strings = " ")
...
```{r echo = T}
tb.cases1 <- tb.cases %>% gather(new_sp_m014:newrel_f65, key = "diag", value = "cases", na.rm = TRUE)
tb.cases1
```
...
```{r echo = T}
tb.cases2 <- tb.cases1 %>% mutate(diag = str_replace(diag, "newrel", "new_rel"))
tb.cases2
```
...
```{r echo = T}
tb.cases3 <- tb.cases2 %>% separate(diag, c("new", "type", "sex.age"), sep = "_")
tb.cases3
```
...
```{r echo = T}
tb.cases4 <- tb.cases3 %>% select(-new, -iso2, -iso3)
tb.cases4
```
...
```{r echo = T}
tb.cases5 <- tb.cases4 %>% separate(sex.age, c("sex", "age"), sep = 1)
tb.cases5
```
...
```{r echo = T}
tb.cases6[sapply(tb.cases6, is.character)] <- lapply(tb.cases6[sapply(tb.cases6, is.character)],as.factor)
tb.cases6
```
...
```{r}
result.summary <- tb.cases6 %>% 
  filter(country %in% c("United States of America", "China", "India", "Brazil", "Indonesia")) %>% 
  filter(year >= 2008) %>% 
  group_by(country, year) %>% 
  summarize(total.cases = sum(cases)) %>% 
  mutate(avg.per.month = total.cases / 12)

print(result.summary, n = 29)
```
