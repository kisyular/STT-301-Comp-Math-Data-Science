...
cor(lsd$concentration, lsd$mathscore)
```
...
```{r}
# Extract the coefficients and the residuals here...
...
```{r}
# Display the coefficients and residuals here...
lsd_lm_coef1
lsd_lm_resid1
```
...
```{r}
# Test whether the extracted coefficients are vectors here...
is.vector(lsd_lm_coef1)
is.vector(lsd_lm_resid1)
```
...
```{r}
# Extract and display the coefficients in the three different ways here...
...
```{r}
# Test whether the coefficient objects are vectors or lists...
```
